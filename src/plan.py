...
DELIVERY_STATION = "mark_1"  # 交付台 / 放置点
...
def navigate_retreat_then_place(
    ...
    *,
    ...
    """抓取后：F→mark_6 / B→mark_7，再到 mark_1，放置位姿，然后 release。"""
...
def shelf_pose_id(location: str) -> str:
    """H*_L1_* → level_1，L2 → level_2，L3 → level_3。"""
...
def run_step1_receipt_to_shelf(
    *,
    ...
    """步骤 1：小票位 → RECEIPT_VIEW → 识别 → 只去第一件（或下一件）货位。"""
...
def run_step3_place(
    *,
    ...
    """步骤 3：后退点 → 放置点 mark_1 → 放置位姿 → release。"""

    print("=== 步骤 3：后退点 → 放置点 → 放置位姿 → release ===")
    state: dict[str, Any] | None = None
    if state_path.is_file():
        state = load_plan_state(state_path)
    location = resolve_location(nav_target, state)
    if location.upper() in {DELIVERY_STATION.upper(), "MARK_1"} and state:
    ...
    *,
    ...
    if go_receipt_station:
        ...
        response = navigate_to(
            ...
            RECEIPT_STATION,
            timeout=nav_timeout,
            idempotency_key=f"plan-{run_id}:receipt-{uuid.uuid4().hex[:8]}",
        )
        elapsed = round(time.time() - started, 1)
        nav_results.append(
            {
                "target_id": RECEIPT_STATION,
                "status": response.get("status"),
                "elapsed_seconds": elapsed,
                "role": "receipt",
            }
        )
        print(f"      到达小票位 ({elapsed}s)")
    if not skip_pose and (go_receipt_station or not skip_receipt):
        print(f"[1b] 小票识别位姿 {POSE_RECEIPT}")
        pose_results.append(
            move_to_receipt_pose(
...
def main() -> int:
    ...
    parser.add_argument(
        "--sku-url",
        default=DEFAULT_SKU_URL,
        help="SKU 服务根地址，默认 %(default)s",
    )
    parser.add_argument(
        "--navigation-url",
        default=DEFAULT_NAVIGATION_URL,
        help="导航服务根地址，默认 %(default)s",
    )
    parser.add_argument(
        "--rgbd-url",
        default=DEFAULT_RGBD_URL,
